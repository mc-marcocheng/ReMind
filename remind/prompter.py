...
prompt_folder_env = Environment(
    loader=FileSystemLoader(
        os.path.join(current_dir, os.environ.get("PROMPT_PATH", "prompts"))
    )
)
...
class Prompter:
    ...
    def setup(self):
        ...
        if self.prompt_template:
            self.template = prompt_folder_env.get_template(f"{self.prompt_template}.jinja")
